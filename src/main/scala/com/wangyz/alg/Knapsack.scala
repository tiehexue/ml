...
object Knapsack {
  ...
  def bad(recursiveTimes: Int, weight: Int, sumW: Int, sumV: Int, dest: List[Item], src: List[Item]): (Int, Int, Int, Int, List[Item], List[Item]) = {
    ...
    if (src.isEmpty || src.head.w > restWeight) {
      ...
      val x = bad(times, weight, sumW + best.w, sumV + best.v, dest :+ best, rests)
      val y = bad(times, weight, sumW, sumV, dest :+ best, rests)  
  ...
  def doIt(sum: Int) = {
    val x = bad(0, sum, 0, 0, List[Item](), items)
